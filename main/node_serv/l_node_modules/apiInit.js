...
let ApiInst={
    init : async  (params, cbp)=>{
        ...
        let extraJson={ limit : '100mb' };
        let extraJUrlEncode={extended: true};

        if (settings.extraJson ){
            extraJson={...extraJson,...settings.extraJson};
        };
        if (settings.extraJUrlEncode ){
            extraJUrlEncode={...extraJUrlEncode,...extraJUrlEncode.extraJUrlEncode};
        };

        app.use(express.json(extraJson));		
        app.use(express.urlencoded(extraJUrlEncode)); 
        ...
        let cors_paramsInit=cors( cors_param ) 
        app.use( "*" , cors_paramsInit );
    
        const io = socketio(http,  { cors : cors_param } );
        mds.vrtw.io=io

        //mds.vrtw.plugins=params.plugins;
    
        let hasPubFE=true
        let FEhtmlpubpath=__dirname + "/" + ".." + "/" + ".." + "/" +  "web_ui" + "/" + "build"
        ...
        let pluginInst=params.plugins.Plugins.init({ 
            vserv : { settings }, 
            params }
        );

        mds.plugins=params.plugins.Plugins;        
        ...
        var gl_bundle={ app : app, express : express, mds : mds, 
            vserv : { settings }, 
            params : params, pub : pub, http : http , io : io, server : server
        }


        let dyn_routes={}
        let dyn_routes_names={}
         //   console.log("gl_bundle app", app)
        let exclusionTypes=["dbJS"];        
        

        feach(mds, function(r1 , l_modulename){                
            let excludeThis=false;
            if (r1.type){
                exclusionTypes.forEach((v1,i2)=>{
                    if (v1===r1.type){
                        excludeThis=true;
                    }
                });
    ...
    mdsInit : async (params, cbp)=>{
        ...
        feach( files  , function(f,i){                    
            let stat=fs.lstatSync(filespath + "/" + f) 
            if (stat.isDirectory()) {
        ...
        for (let i in auto_mod_folders){
            let file=auto_mod_folders[i];

            let temp_DIR = path.join(filespath, file);
            
            let exclusionTypes=["dbJS"];    

            let mdsTmp=await ApiInst.autoLoadModules(temp_DIR,{ 
                vserv :  settings, 
                mds : {
                    vrtw : { 
                            //db : params.db,
        ...
        mds.vrtw={}
        if (true){
            mds.vrtw.db=params.gdb.db
            mds.vrtw.dbA=params.dbA
            mds.vrtw.dbAdmin=params.dbA
    ...
    autoLoadModules: async function  (fpath, paramsIn , cbIn) {   
        let debug=false;
        
        let mds={};
        let skip_exp_name=false;
        let params={}
        if ( !isUn(arguments[1]) ){
            if (isOb(arguments[1])){
                params=arguments[1]
        ...
        let cb; // leave undefined as an unitialised check later 
        if ( params.cb){
            cb=params.cb
        }
        if (cbIn){
                ...
                if (f.endsWith(".js")){ 
                    ...
                    if (skip_exp_name ){ // dont use export name , just use export library values id object has been exported 
                        ...
                        feach(temp, function(val,prop){
                            ...
                            if ( typeof(params.cbReplace)==="function"){ 
                                let recCb={ 
                                    mds,
                                    temp,
                                    val,
                                ...
                                let tmp=params.cbReplace(recCb);
                                ...
                                if (params.mustConstainProp){
                                    if (containsType){
                                        ...
                                        feach(temp, function(val,prop){   
                                            if (mds[prop]){
                                                if (debug){
                                                    console.log("more then one module with same name : ",prop );
                                                    console.log("renaming_to : ",prop + "__dupl__" + duplmod);
                                                }
                                                mds[prop + "__dupl__" + duplmod]=val;
                                                duplmod++
                                            }else{
                                                mds[prop]=val
                                            }
                                        ...
                                        if (containsAutoRun){
                                            return mds[containsAutoRunProp][params.mustRunFn](params)
                                        }
                                    }
                                }else{
                                    if (params.exludeTypes){
                                        let duplmod=1;
                                        feach(temp, function(val,prop){  
                                            let r1=temp[prop];
                                            let excludeThis=false;
                                            
                                            params.exludeTypes.forEach((v1,i2)=>{
                                                if (v1===r1.type){
                                                    excludeThis=true;
                                                }
                                            });
                                        ...
                                        // do all of them without question 
                                        let duplmod=1;
                                        feach(temp, function(val,prop){  
                                            if (mds[prop]){
                                                if (debug){
